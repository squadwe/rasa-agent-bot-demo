...
{  
  ...
  $create_message = send_to_squadwe($account, $conversation, $bot_response->text);
}
...
function send_to_squadwe($account, $conversation, $message){
  global $squadwe_url, $squadwe_bot_token; 
  $url = "{$squadwe_url}/api/v1/accounts/{$account}/conversations/{$conversation}/messages";
  ...
  'http' => array(
      ...
      'header'=>  "Content-Type: application/json\r\n" .
                  "Accept: application/json\r\n" .
                  "api_access_token: {$squadwe_bot_token}"
      )
  );

  $context  = stream_context_create( $options );
  $result = file_get_contents( $url, false, $context );
  error_log("squadwe response: {$result}",0);
  $response = json_decode($result);
  return $result;
}
